...
│   └── datos_simulacion_MPC_MPC_Cambios.csv
...
| Métrica | MPC (Cambios) | EGT | ESC (Adaptativo) |
| :--- | :--- | :--- | :--- |
...
*   **Precisión ($\text{E}_{ss}$):** El **EGT** demuestra la mayor precisión en estado estacionario ($0.01 \text{ °C}$), lo cual es un éxito directo de su función de pago asimétrica, diseñada específicamente para este fin.
*   **Eficiencia Energética:** El **ESC** cuenta con una eficiencia energéticamente similar a la del EGT ($6.23 \text{ MJ}$ en un solo salón), confirmando que la búsqueda adaptativa de un punto óptimo es ideal para reducir el consumo total.
